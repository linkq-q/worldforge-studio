...
api.place({assetId,name?,position:[x,y,z],terrain:false,rotationY?:radians,scale?:number|[sx,sy,sz],role?:'structure',groupId?,layer?:1|2|3|4}) returns a placement reference. Y-up; position y is the object's bottom height, x/z its horizontal center. Local Z+ is front and X+ right. rotationY uses radians: a quarter turn is Math.PI/2, never 90. A yaw rotates local X+ to [cos(yaw),-sin(yaw)] and local Z+ to [sin(yaw),cos(yaw)] in world x/z. Scale multiplies actual catalog dimensions, not target dimensions. Use measured dimensions below when planning contacts. Do not treat a placement reference as an asset ID.
