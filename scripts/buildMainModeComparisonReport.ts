...
type Run = { name: string; prompt: string; seed: number; full: string; minimal: string;
  ...
  earlierAttempts?: { mapId?: string; error?: string }[]; retest?: Run; improved?: Run; terrain?: Run };
...
type Review = { winner: 'full' | 'minimal' | 'main' | 'improved' | 'terrain' | 'tie' | 'undecided'; note: string };
...
const labels = { full: '旧全规划 + DeepSeek', minimal: '旧精简规划 + DeepSeek',
  ...
  terrain: '本轮地形聚落主模式 + DeepSeek' } as const;
...
for (const file of readdirSync(traceDir).filter(name => name.endsWith('.jsonl'))) {
  ...
  try {
    ...
    if (!/^main-mode-(comparison|retest)-/.test(operation)
      && operation !== 'improved-main-comparison-plan') continue;
    const events = lines.map(line => JSON.parse(line));
    if (operation === 'improved-main-comparison-plan') {
      ...
      if (typeof code === 'string') {
        const list = improvedAttempts.get(first.data.scene) ?? [];
        list.push({ at: first.at, attempt: Number(first.data.attempt), code });
        improvedAttempts.set(first.data.scene, list);
      }
      continue;
    }
...
const sections = runs.map((run, index) => {
  const review = terrainReviews[run.name] ?? improvedReviews[run.name] ?? reviews[run.name];
  ...
  const cards = modes.map(mode => {
    const current = mode === 'terrain' ? run.terrain : mode === 'improved' ? run.improved : mode === 'main' ? latest : undefined;
    const id = current ? (current.status === 'saved' ? current.mapId : undefined) : run[mode as 'full' | 'minimal'];
    const status = mode === 'terrain'
      ...
      : mode === 'main'
        ...
        : '旧记录';
    return `<article class="card"><div class="cardhead"><h3>${labels[mode]}</h3><span>${escape(status)}</span></div>
  ...
  return `<section id="scene-${index + 1}"><div class="sectionhead"><span class="number">${String(index + 1).padStart(2, '0')}</span><div><h2>${escape(run.name)}</h2><p>原提示词：${escape(run.prompt)}</p><small>尺寸 96 × 16 × 96 · 新图种子 ${run.seed}${index === 0 ? '；旧精简图种子另为 1431999235' : ''}</small></div></div>
    <div class="grid${run.improved ? ' four' : ''}"${run.terrain ? ' style="grid-template-columns:repeat(auto-fit,minmax(250px,1fr))"' : ''}>${cards}</div>
    <div class="assessment"><div><strong>${run.terrain?.status === 'saved' && terrainReviews[run.name] ? '五图判断' : run.improved?.status === 'saved' && improvedReviews[run.name] ? '四图判断' : '当前可见图判断'}：${escape(winner)}</strong><p>${escape(review?.note ?? '新图或截图尚未齐备，暂不判定。')}</p></div>
      <label>你的评语<textarea data-scene="${escape(run.name)}" placeholder="在这里写下你的观察。内容保存在当前浏览器；可用页面顶部按钮导出。"></textarea></label></div>
    <div class="evidence">${prior}${codeDetails}${improvedEvidence}${terrainEvidence}</div>
  </section>`;
}).join('');
...
</ol><p>上段记录的是上一轮生成时的规则。随后改进轮已移除单个资产提示词 1200 字符硬上限，并恢复建筑模块与聚落布局引导；因此新一轮结果还受到提示词和执行规则变化影响。先前的本地重放为 19 次中 18 次可执行，剩余一次资产提示词长达 1494 字符，在当时的硬上限处失败。之后对原先失败的 7 个场景重新调用 GPT 和 DeepSeek，7 个新首版均成功保存。重测没有启用可选代码或空间修复；历史失败与原首版代码仍在本页保留。日志只够统计请求尝试，不能计算真实费用。</p>${improvedDiagnosis}</section>`;
