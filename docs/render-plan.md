...
- `runtime.water-style`：湖泊/河流配方、透明度、颜色、波纹、泡沫和反射强度
...
- 水体可作用于带 `water` 标签的模型水面；结构化湖泊/河流接入后沿用同一协议。
- 景深目前只有高层协议，宿主暂不执行。
...
开发者模式同时显示预设当前值。数值参数使用“滑条 + 精确数值框 + 当前值”，修改后立即刷新画面；滑条边界来自该方案的开发者范围。保存始终生成新方案，不覆盖内置预设。
