...
`RenderPlan.visualDirection` 是可选的高层视觉单一真值，保存对比模式、早/中/晚、冷暖、语义色板和环境特效意图。旧方案不含该字段时继续按原模块工作；新方案先由视觉导演编译高层意图，再由显式模块做受控覆盖。地图自身通过 `EditableMap.visualSemantics` 保存区域标签与共享风场，空间语义不塞进渲染方案，因此同一方案可以复用于不同地图。

颜色导演位于 `src/shared/colorDirector.ts`。它把语义色板协调到背景、环境光、太阳光、距离雾、色彩分级、卡通暗部、水体深浅色，以及已声明的材质 Tag 主题。HDRI 上下半球取色会保留贴图本身的色彩身份，但优先级低于视觉导演；开发者或用户显式写入的模块参数始终最高，不会被自动协调覆盖。
