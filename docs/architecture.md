...
Scene composition also compiles map-owned `visualSemantics`: bounded world-space zones tagged as grass, forest, water, lowland, dry, settlement or rocky, plus one shared wind field. Render schemes own only high-level atmosphere intent. `compileAtmosphereFx` combines both sources into weak semantic defaults and explicit boosts; region particles, grass and water consume the same wind direction and strength. Particles are capped to three draw calls.

`runtime.atmosphere-fx` is the explicit override layer. Ordinary users get one master-strength slider; developer mode exposes semantic baseline and per-channel values through the same capability/access-policy system, so a virtual control becomes a real plan module only after it is edited. The editor's adaptive quality controller uses sustained-frame hysteresis: it first lowers atmosphere particle budgets and pixel ratio after two seconds below target, and restores them only after six seconds of healthy frames. The debug panel reports live RuntimeIndex batching coverage, effect re-batches, fallback parts, atmosphere draws and the active quality scale.
...
Ocean coast rendering derives bounded smoothed contours and a local distance-driven slope from the authored height field without persisting those changes. Terrain vertices, ocean depth/shore textures and tidal wet-sand shading share the derived surface. The deep ocean becomes opaque before the render-only seabed continues down to its hidden floor, so distance fog can remain tied to real scene depth instead of inserting an analytic ocean plane in front of map objects. This does not change the depth resource consumed by water, non-ocean rendering, or saved render-scheme settings.
...
湖泊和河流都不是浮在地形上的平面：`water.add` 与 `water.update` 会把盆地或河床刻进高度场。湖泊用平滑后的圆弧岸线和确定性噪声生成不规则闭合边界；河流用从上游到下游的中心线与沿程水位生成倾斜水面、河床和缓坡河岸。刻蚀对每个格点取“当前高度与目标上限的较小值”，因此重复执行不会越挖越深；缩小或删除水体会留下已有洼地，需要用笔刷填回。高度场允许负值，`y=0` 是海平面而不是地板。

地图创建使用小（48×12×48，33² 高度场）、中（96×16×96，65²）和大（192×24×192，129²）三档。AI 的笔刷、水域、物体和资产配额根据地图面积计算。场景层把摆放意图区分为 `anchor/field/patch/linear/layout/attached`：旧的 `accent/even/clustered` 分别兼容映射到前三类；围栏、路灯和沿路物体沿区块导向轴按固定间距采样，建筑可按行列、院落、放射或网格槽位生成，附属物围绕已落地的目标家族生成。重复植被或岩石继续使用确定性散布，但相关家族共享宏观群落 seed、保留各自微观变化，并可声明物种间距矩阵以及高度、坡度、距水距离适宜带。所有模式都拒绝水域、陡坡、间距不足或与已有碰撞体重叠的候选点，并按建筑结构、自然群落、附属物的顺序共享已占用空间。出生点若落在水域、陡坡或物体碰撞范围内，会在请求点附近寻找安全位置。高级摆放意图不进入持久化事务协议；预览前统一展开成普通 `object.add`，确保预览与提交坐标完全相同。
...
每张地图保存一个全局 `seed`；旧地图按地图 ID 确定性补齐。AI 先输出一个高层 `terrain.generate` 基础高度场，再用 `mountain/ridge/valley/basin/cliff/terrace/dune/island` 做局部塑形。山体必须声明 `walkable` 或 `scenic`：高度按区域最窄宽度限幅，小区域山脊自动降级为宽缓山丘；`terraces` 平台层高受玩家跳跃高度约束。峭壁由带方向的宏观断层、两级崖线扰动、崖顶/崖脚肩部和有限地层细节共同形成，即使 `softness=0` 也保留最小自然过渡，不生成单格刀切。所有塑形完成后可执行一次 `terrain.refine`：受休止角限制的热侵蚀只搬运过陡坡面的高度，D8 汇流只轻量刻蚀已有排水路径；随后才执行湖泊/河流刻蚀和对象摆放。LLM 只选择这些白名单语义与有界参数，不枚举基础地形坐标。渲染端只消费最终高度场，并按高度、坡度和水线生成顶点颜色。

渲染生成同样使用受限协议，而不是让模型直接修改 Three.js：AI 选择基础方案并组合 `RenderPlan` 能力模块，服务端按该方案的 `accessPolicy` 校验模块、参数、角色权限和范围。非法结果或明确风格遗漏只允许自动修正一次。当前模块覆盖环境、HDRI 天空、表面、描边、世界空间素描、漫画、色彩分级、标签材质、命名灯光、Bloom/SSAO、标签特效，以及结构化湖泊/河流与模型水面。生成请求携带精简的 `RenderSceneProfile`，让同一协议根据室内/室外、门窗、材质和实际灯光覆盖选择确定性默认；室内使用中性 PBR 基底、默认无全局距离雾并启用柔和 SSAO。`environment.hdri` 的贴图来自 `data/map-editor/hdri/` 目录扫描，同一张全景图可以独立控制背景可见性，并经应用曝光、饱和度、染色和独立环境强度后的 PMREM 成为 `scene.environment`；室内暂时继续从同一池选择，但隐藏天空球并只保留克制的材质反射。环境变更由宿主桥同步给 Voxel 材质标签表面绑定和 `WaterSurface`，清除 HDRI 时也同步解绑。距离雾统一使用 Runtime 的深度后处理，覆盖普通材质、透明水体和风格化输出，避免自定义 Shader 绕开 Three 内建雾；水体参与该深度预通道，但仍不进入普通材质替换。统一描边的距离淡出使用世界米制阈值，默认从 120m 到 260m，且只向开发者开放。水体复用 HDRI 环境反射，切换方案时重新绑定，并独立限制反射强度和曝光以避免亮天空洗白水色。贴图名是开发者专用参数，AI 只能从扫描到的池中选择，并调旋转、曝光、饱和度、天空强度、环境强度和色调。材质与特效只接收 `modelsRoot`；后处理 normal/depth 预通道只接收地图内容根，物体/材质 ID 只遍历 `modelsRoot`，编辑器 gizmo 和辅助物不会进入风格链。景深仅保留高层协议。
...
有地形的非海洋水面另带独立的世界空间水柱深度场，颜色和透光按河床深度计算；岸线距离仍只负责轮廓相关效果。室内隐藏地形、地图外及 `carveTerrain:false` 的结构支撑水段不采纳地形深度。地形笔刷同步刷新水深纹理。地图水通过既有 `WaterSceneCapture` 共享一次半分辨率不透明场景颜色/深度捕获，在 `RenderFrameCoordinator` 的资源图中按需调度；不会按水体数量增加捕获次数。细节法线共享小型可平铺纹理，并沿河流方向循环推进，太阳高光使用场景实际方向和强度。无地形模型水保留原有默认参数。
...
两种模式共享 `MapOperation[]` 与验证规则，通过同一个事务入口原子提交。当前支持最近一次事务的单步撤销/重做；接入 API 模型与外部 Agent 时不得再生成第二套地图修改逻辑。
...
- 多级 AI/Agent 事务历史；实验阶段只保留最近一次事务的单步撤销/重做
