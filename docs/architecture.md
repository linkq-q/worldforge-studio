...
Ocean coast rendering derives bounded smoothed contours and a local distance-driven slope from the authored height field without persisting those changes. Terrain vertices, ocean depth/shore textures and tidal wet-sand shading share the derived surface. The deep ocean becomes opaque before the seabed mesh ends; distance fog caps underwater/empty-background rays at the ocean surface instead of outlining the hidden seabed through its opaque-only depth input. This does not change the depth resource consumed by water, non-ocean rendering, or saved render-scheme settings.
