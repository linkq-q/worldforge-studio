...
空地图生成接口内部运行有界场景编排：总导演输出 `SceneCompositionPlan`；按导演决策调用 0-2 个局部专家；服务端按语义标签和地图级建模模式解析资产，按小/中/大地图最多生成 3/5/8 类缺失资产；确定性编译器展开地形、水体和散布；合成审查最多修正一次计划；最后 `mapLint` 验证编译后的真实地图状态。任一模型角色的无效 JSON 最多修复一次，不存在无限 Agent loop。
