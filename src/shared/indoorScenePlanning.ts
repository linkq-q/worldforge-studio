...
interface IndoorAssetDemand {
  ...
  placement: 'anchor' | 'wall' | 'group' | 'paired' | 'social';
  targetKey?: string;
...
export function completeIndoorScenePlan(
  ...
  const target = indoorAssetTargetCount(map, minimumAssets, maximumAssets);
  const demands = indoorDemandTree(`${prompt} ${plan.summary} ${plan.globalBrief.spatialTheme}`);
...
function demandLayer(
  ...
  const mode = demand.placement === 'wall'
    ? 'linear' as const
    : demand.placement === 'paired' || demand.placement === 'social'
      ? 'attached' as const
  ...
  return {
    ...
    scaleRange: [1, 1.15],
    distribution: demand.placement === 'anchor' ? 'accent' : demand.placement === 'paired' || demand.placement === 'social' ? 'clustered' : 'even',
    edgeFalloff: 0.12,
    placement: {
      mode,
      ...
      maxPerGroup: demand.maxPerGroup ?? (demand.placement === 'wall' || demand.placement === 'anchor' ? 1 : demand.placement === 'paired' ? 1 : 4),
...
function restaurantDemands(): IndoorAssetDemand[] {
  return [
    demand('dining-table', 'Dining table', 'repeated dining group anchor', ['dining-table', 'restaurant', 'furniture'], /dining table|restaurant table|餐桌|饭桌/i, 'medium', 0.96, 'One broad cartoon voxel dining table for a small restaurant; single reusable table only.', 'group', undefined, 0, 12),
    demand('dining-chair', 'Dining chair', 'chairs arranged around every dining table', ['dining-chair', 'restaurant', 'furniture'], /dining chair|restaurant chair|餐椅/i, 'medium', 0.95, 'One broad sturdy dining chair matching the dining table.', 'social', 'dining-table', 0, 4),
