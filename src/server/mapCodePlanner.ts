...
Anchor a local activity cluster once, then place its related props in host-local coordinates when geometry permits: table with chairs, quay with mooring gear, workshop with tools. If the anchor is route-derived, keep only the anchor tied to its route; do not independently bind every child to a street. When no safe host geometry is known, calculate nearby world positions for separate props without a sourceGuideId; keep their authored arrangement rather than forcing a parent link.
Leave sourceGuideId unset for freely scattered scenery, including habitat plants, rocks, accent lights and benches not meant to line a specific route. Use region, habitat and bounded spacing to compose their distribution; a noun such as lamp or bench does not imply street ownership. Only route-derived placements should claim a source route.
...
Generic assembly derivation:
1. Select the spatial organization, footprint family, massing profile, structural rhythm and functional sequence from the requested use and site.
2. Derive one or more connected boundary runs from that footprint, preserving intentional entrances, courts and passages.
3. Subdivide each run by the canonical span of its chosen structural module; reserve special bays for corners, thresholds and vertical circulation.
4. Expand the building with bounded nested loops in this order: for each massing tier -> for each story -> for each boundary run -> for each bay.
5. Choose the entrance, corner, ordinary, roof or cornice module by the bay's role; place continuous runs with shared endpoints and gapRatio:0, then add only the secondary supports the chosen rhythm requires.
Write the actual bounded JavaScript for the selected geometry. Do not copy a showcased outline or fixed bay count merely because it is familiar.
