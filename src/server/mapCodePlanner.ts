...
export function buildMapCodePlannerSystemPrompt(
  ...
  const scopeContract = requestMode === 'refine'
    ...
    : '';
  const compositionContract = promptMode === 'coupled' && requestMode === 'generate' && scope === 'scene'
    ? `\n## Shared generative relationships for a large scene\nWrite one executable scene program whose parts depend on the same few spatial decisions. Define shared anchors, paths and helper functions first; reuse their coordinates and distances for terrain, water edges, routes, focal objects, vegetation and repeated details. For example, one lake shoreline can govern wetness, path setback, planting density and viewpoints. Do not draw disconnected coordinate lists for each use.\nUse smooth scalar fields over (x,z) when a property should vary continuously: distance to water or paths, openness, slope, focal pull, noise and density. Combine them with bounded arithmetic and smooth transitions. Evaluate the fields through api.sampleProbabilityField for point placement, with finite candidates, maxPoints and minDistance; use marks only when several object families share a distribution. Define assets once and place them from sampled points. Call this after the terrain, water and routes it reads have been declared. Avoid per-meter map scans, nested placement loops and arbitrary region cuts.\nUse small reusable motif functions for local repeated structures; a bounded candidate score is useful only when two plausible anchors or layouts need choosing. Functional uses may overlap and emerge from the shared fields. Hard edges and named regions are appropriate only where the requested world truly has a wall, shoreline, room or other physical boundary. Keep major requested destinations legible and routes connected while preserving gradual transitions and intentional empty space.\n`
    : '';
  const densityContract = promptMode === 'coupled' && requestMode === 'generate' && scope === 'scene'
    ? `\n## Area-scaled placement budget\nEstimate useful object counts from walkable area, ecological coverage and each family's spacing before setting sampler quotas. The 128-point sampler default is not a whole-map target. A detail-rich 192m by 192m botanical garden may need 500 or more purposeful placements across canopy, shrubs, flowers and built details; a sparse scene may need fewer. Allocate separate bounded samplers to distinct strata when the total exceeds one call's 512-point limit, and allow enough candidates for each field to reach its quota. Keep clear routes and intentional open space; never add filler just to hit a number.\n`
    : '';
  return `You are WorldForge Studio's procedural environment planner.${scopeContract}${compositionContract}${densityContract}
...
function normalizeCodeEnvironmentOptions(value: unknown): MapEnvironmentSampleOptions {
  ...
  if (input.waterId !== undefined && typeof input.waterId !== 'string') throw new Error('invalid_map_code_environment_water_id');
  const waterId = typeof input.waterId === 'string' ? input.waterId.trim() : undefined;
  if (waterId !== undefined && (!waterId || waterId.length > 80)) throw new Error('invalid_map_code_environment_water_id');
  return {
