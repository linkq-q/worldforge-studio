...
export function buildSceneDirectorPrompt(
  ...
  return [
    ...
    indoor
      ...
      : `Terrain capabilities are composable and independent: ${JSON.stringify(terrainCapabilitySummary())}. Choose a global terrainBase and terrainRefinement, then add a zone terrain.modifier and terrain.surface when the request calls for ridges, valleys, basins, cliffs, terraces, dunes, islands, sand, grass, rock, soil, or paving. For example, a mountain can use a cliff modifier even when the base is hills; never assume a modifier belongs only to one preset.`,
    indoor
    ...
    'Use Simplified Chinese for user-facing labels and summaries. assetFamilies[].label and grassFamilies[].label must each be one short 2-8-character Chinese noun phrase; ids, tags, roles, and generationBrief may remain technical English.',
    ...
    `Execution budgets: about ${limits.objectCount} objects and ${options.minNewAssets ?? 0}-${options.maxNewAssets ?? limits.assetRequestCount} newly generated asset families. Variants of one family count as one toward this range but require separate model calls; define enough useful families to satisfy the minimum.`,
