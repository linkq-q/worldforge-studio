...
export class TriplanarLayer extends EffectLayer {
  ...
  getUniformDeclarations() {
    return [
      ...
      '  return vec3(field1, field2, lineW);',
      '}',
  ...
  getNormalBody() {
    return [
      ...
      '  float marbleLineW = triMarbleData.z;',
      '  float marbleAa1 = max(fwidth(triMarbleData.x), 1e-4);',
      '  float marbleAa2 = max(fwidth(triMarbleData.y), 1e-4);',
      '  float marbleLine1 = (1.0 - smoothstep(0.0, marbleLineW + marbleAa1, abs(triMarbleData.x - 0.5))) * min(1.0, marbleLineW / marbleAa1);',
      '  float marbleLine2W = marbleLineW * 0.85;',
      '  float marbleLine2 = (1.0 - smoothstep(0.0, marbleLine2W + marbleAa2, abs(triMarbleData.y - 0.55))) * min(1.0, marbleLine2W / marbleAa2);',
      '  triMarbleData.z = clamp(marbleLine1 + marbleLine2 * 0.7, 0.0, 1.0);',
  ...
  getFragmentBody() {
    return [
      ...
      '} else if (uTriplanarPattern < 7.5) {',
      '  // ===== FABRIC VERTICAL STRIPES：塑料棚布/遮阳棚，条带沿竖向延伸 =====',
      '  float stripePeriod = max(uTriplanarPlankScale, 0.04);',
      '  float stripeCell = fract(triUv.x / stripePeriod);',
      '  float stripeEdge = max(fwidth(stripeCell) * 1.25, 0.015);',
      '  float stripeMask = smoothstep(0.5 - stripeEdge, 0.5 + stripeEdge, stripeCell);',
      '  triTint = mix(uTriplanarColorLo, uTriplanarColorHi, stripeMask);',
      '} else if (uTriplanarPattern < 8.5) {',
      '  // ===== FABRIC HORIZONTAL STRIPES：横向棚布/围挡条纹 =====',
      '  float stripePeriod = max(uTriplanarPlankScale, 0.04);',
      '  float stripeCell = fract(triUv.y / stripePeriod);',
      '  float stripeEdge = max(fwidth(stripeCell) * 1.25, 0.015);',
      '  float stripeMask = smoothstep(0.5 - stripeEdge, 0.5 + stripeEdge, stripeCell);',
      '  triTint = mix(uTriplanarColorLo, uTriplanarColorHi, stripeMask);',
      '} else {',
      '  // ===== FABRIC CHECKER：双色格纹塑料布/市集棚布 =====',
      '  float checkSize = max(uTriplanarPlankScale, 0.04);',
      '  vec2 checkCell = floor(triUv / checkSize);',
      '  float checkMask = mod(checkCell.x + checkCell.y, 2.0);',
      '  triTint = mix(uTriplanarColorLo, uTriplanarColorHi, checkMask);',
      '}',
      '',
      'float triPatternIsAbsolute = step(6.5, uTriplanarPattern);',
      'triTint *= mix(triMacroShade * triFineShade, 1.0, triPatternIsAbsolute);',
      'vec3 triLighting = clamp(gl_FragColor.rgb / max(diffuseColor.rgb, vec3(0.08)), vec3(0.0), vec3(3.0));',
      'vec3 triResult = mix(gl_FragColor.rgb * triTint, triTint * triLighting, triPatternIsAbsolute);',
