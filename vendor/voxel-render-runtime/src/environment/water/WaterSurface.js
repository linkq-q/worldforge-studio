  ...
  void main() {
    ...
    if (uRippleDecalEnabled && uRippleDecalCount > 0.0) {
      ...
      for (int i = 0; i < ${MAX_RIPPLE_DECALS}; i++) {
        ...
        float rippleTime = birthTime < 0.0 ? uTime : uTime - birthTime;
        float lifetime = birthTime < 0.0 ? uRippleDecalLifetime : uRippleDecalLifetimes[i];
        if (birthTime >= 0.0 && (rippleTime < 0.0 || rippleTime >= lifetime)) continue;
        ...
        float ring = ringStripe(d, uRippleDecalFrequency, uRippleDecalSpeed, uRippleDecalWidth, rippleTime) * rangeMask * env * frontMask;
        ...
        float wave = sin((d * uRippleDecalFrequency - rippleTime * uRippleDecalSpeed) * 6.2831853);
